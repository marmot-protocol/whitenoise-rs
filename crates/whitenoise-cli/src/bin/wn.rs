...
use whitenoise_cli::config::Config;
use whitenoise_cli::protocol::Request;
use whitenoise_cli::{client, output};
...
async fn main() -> whitenoise_cli::Result<()> {
    ...
    match args.command {
        ...
        Cmd::Reset { confirm } => {
            if !confirm {
                return Err(whitenoise_cli::CliError::msg(
