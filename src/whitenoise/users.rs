...
mod tests {
    ...
    async fn test_resolve_user_dedupes_background_resolution_for_same_unknown_user() {
        ...
        // Wait for the background task to run at least once, then verify deduping happened.
        ...
        .await
        .expect("background resolution should run");

        let run_count = whitenoise.user_resolution_run_count_for_testing(&pubkey);
        assert!(run_count >= 1);
        assert!(run_count <= 2);
    }
