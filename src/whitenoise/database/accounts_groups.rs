...
impl AccountGroup {
    ...
    /// Returns the MLS group IDs of all visible non-DM groups for an account.
    ///
    /// Performs a single JOIN query against `accounts_groups` and `group_information`
    /// to filter out `direct_message` groups at the database layer, avoiding N+1 queries
    /// when the caller only needs non-DM groups.
    ///
    /// Groups that have no `group_information` row are excluded (the row is always created
    /// eagerly at group creation and welcome time, so this is safe for current data).
    ///
    /// # Arguments
    ///
    /// * `account_pubkey` - The account whose groups to query
    /// * `database` - A reference to the `Database` instance
    ///
    /// # Returns
    ///
    /// Returns a `Vec<GroupId>` of visible (pending + accepted) non-DM group IDs.
    pub(crate) async fn find_visible_non_dm_group_ids_for_account(
        account_pubkey: &PublicKey,
        database: &Database,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_find_visible_non_dm_group_ids_returns_only_groups() {
        use crate::whitenoise::group_information::{GroupInformation, GroupType};

        let (whitenoise, _data_temp, _logs_temp) = create_mock_whitenoise().await;
        let account = whitenoise.create_identity().await.unwrap();
        let group_id = GroupId::from_slice(&[80; 32]);
        let dm_group_id = GroupId::from_slice(&[81; 32]);

        // Regular group
        GroupInformation::create_for_group(&whitenoise, &group_id, Some(GroupType::Group), "Team")
            .await
            .unwrap();
        AccountGroup::find_or_create(&account.pubkey, &group_id, None, &whitenoise.database)
            .await
        ...
        .await
        .unwrap();
        AccountGroup::find_or_create(&account.pubkey, &dm_group_id, None, &whitenoise.database)
            .await
            .unwrap();

        let ids = AccountGroup::find_visible_non_dm_group_ids_for_account(
            &account.pubkey,
            &whitenoise.database,
        )
        .await
        ...
        assert_eq!(ids.len(), 1);
        assert_eq!(ids[0], group_id);
    }

    #[tokio::test]
    async fn test_find_visible_non_dm_group_ids_excludes_declined() {
        use crate::whitenoise::group_information::{GroupInformation, GroupType};

        let (whitenoise, _data_temp, _logs_temp) = create_mock_whitenoise().await;
        let account = whitenoise.create_identity().await.unwrap();
        let group_id_accepted = GroupId::from_slice(&[82; 32]);
        let group_id_declined = GroupId::from_slice(&[83; 32]);

        for gid in [&group_id_accepted, &group_id_declined] {
            GroupInformation::create_for_group(&whitenoise, gid, Some(GroupType::Group), "G")
                .await
                .unwrap();
        }

        let (ag_accepted, _) = AccountGroup::find_or_create(
            &account.pubkey,
            &group_id_accepted,
            None,
            &whitenoise.database,
        ...
        let (ag_declined, _) = AccountGroup::find_or_create(
            &account.pubkey,
            &group_id_declined,
            None,
            &whitenoise.database,
        ...
        assert_eq!(ids.len(), 1);
        assert_eq!(ids[0], group_id_accepted);
    }

    #[tokio::test]
    async fn test_find_visible_non_dm_group_ids_excludes_groups_without_info_row() {
        let (whitenoise, _data_temp, _logs_temp) = create_mock_whitenoise().await;
        let account = whitenoise.create_identity().await.unwrap();
        let group_id = GroupId::from_slice(&[84; 32]);

        // Create accounts_groups row but NO group_information row
        AccountGroup::find_or_create(&account.pubkey, &group_id, None, &whitenoise.database)
            .await
            .unwrap();
        ...
        // No group_information row → excluded by INNER JOIN
        assert!(ids.is_empty());
    }
