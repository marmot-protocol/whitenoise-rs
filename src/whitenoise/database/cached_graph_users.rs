...
impl CachedGraphUser {
    ...
    pub(crate) async fn find_fresh(
        pubkey: &PublicKey,
        database: &Database,
    ) -> Result<Option<CachedGraphUser>, WhitenoiseError> {
    ...
    pub(crate) async fn find_fresh_with_ttl(
        ...
        Ok(row.map(CachedGraphUser::from))
    }
    ...
    pub(crate) async fn find_fresh_batch(
        pubkeys: &[PublicKey],
        database: &Database,
    ) -> Result<Vec<CachedGraphUser>, WhitenoiseError> {
    ...
    pub(crate) async fn find_fresh_batch_with_ttl(
        ...
        Ok(rows.into_iter().map(CachedGraphUser::from).collect())
    }
    ...
    pub(crate) async fn upsert(
        &self,
        database: &Database,
    ) -> Result<CachedGraphUser, WhitenoiseError> {
        ...
        let row = sqlx::query_as::<_, CachedGraphUserRow>(
            "SELECT * FROM cached_graph_users WHERE pubkey = ?",
        )
        .bind(&pubkey_hex)
