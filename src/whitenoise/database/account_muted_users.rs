...
impl AccountMutedUsers {
    ...
    pub(crate) async fn replace_all_for_account(
        ...
        let unique_muted: HashSet<String> = muted.iter().map(PublicKey::to_hex).collect();

        for pk_hex in unique_muted {
            sqlx::query(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_replace_all_deduplicates_input() {
        let (whitenoise, _d, _l) = create_mock_whitenoise().await;
        let account = Keys::generate();
        let a = Keys::generate();
        insert_test_account(&whitenoise.database, &account.public_key()).await;
        ...
        let (count,): (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM account_muted_users WHERE account_pubkey = ? AND muted_pubkey = ?",
        )
        .bind(account.public_key().to_hex())
        .bind(a.public_key().to_hex())
