    ...
    pub(crate) async fn handle_mute_list(&self, account: &Account, event: Event) -> Result<()> {
        ...
        .await?
        {
            ...
            for ag in pending {
                ...
                ag.decline(self).await.map_err(|error| {
                    tracing::warn!(
                        target: "whitenoise::handle_mute_list",
                    ...
                    WhitenoiseError::MuteList(format!(
                        "failed to auto-decline invite for group {}: {}",
                        hex::encode(ag.mls_group_id.as_slice()),
                        error
                    ))
                })?;
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_handle_mute_list_decline_failure_is_retryable_and_not_tracked() {
        let (whitenoise, _data_temp, _logs_temp) = create_mock_whitenoise().await;
        let account = whitenoise.create_identity().await.unwrap();
        let keys = whitenoise
            .secrets_store
        ...
        sqlx::query(
            "CREATE TRIGGER fail_decline
             BEFORE UPDATE OF user_confirmation ON accounts_groups
             WHEN NEW.user_confirmation = 0
             BEGIN
                 SELECT RAISE(FAIL, 'forced decline failure');
             END;",
        )
        ...
        let ts = Timestamp::now();
        let event = build_mute_list_event(&keys, &[welcomer], Some(ts)).await;
        let result = whitenoise.handle_mute_list(&account, event.clone()).await;

        assert!(result.is_err());
        assert!(matches!(result, Err(WhitenoiseError::MuteList(_))));
