...
pub struct MatchResult {
    /// Whether any field matched the query.
    pub matches: bool,
    ...
    /// All field names that matched - useful for highlighting in UI.
...
pub fn match_metadata(metadata: &Metadata, query: &str) -> MatchResult {
    ...
    let fields: [(MatchedField, &str, Option<&str>); 4] = [
        (MatchedField::Name, "name", metadata.name.as_deref()),
        (MatchedField::Nip05, "nip05", metadata.nip05.as_deref()),
        (
            MatchedField::DisplayName,
            "display_name",
            metadata.display_name.as_deref(),
        ),
        (MatchedField::About, "about", metadata.about.as_deref()),
    ];
    ...
    for (field_enum, field_name, field_value) in fields {
...
mod tests {
    ...
    fn multiple_fields_match_all_recorded() {
        ...
        assert!(result.matched_fields.contains(&"name".to_string()));
        assert!(result.matched_fields.contains(&"display_name".to_string()));
        assert!(result.matched_fields.contains(&"about".to_string()));
    }
    ...
    fn whitespace_only_query_matches_nothing() {
        ...
        assert!(!match_metadata(&metadata, "   ").matches);
        assert!(!match_metadata(&metadata, "\t").matches);
