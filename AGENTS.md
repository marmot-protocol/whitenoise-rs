...
`crates/whitenoise-frb/` (the FFI wrapper crate) and published as a hosted
Dart pub package `whitenoise_frb` to a static registry on this repo's
`gh-pages` branch on every codeowner push to `master`. The Flutter app
consumes it via a standard `hosted:` pubspec source. No codegen runs on the
Flutter side.

**Master owns the wrapper crate and the package template.** The Flutter
package shape (pubspec, cargokit, platform dirs) lives at
`crates/whitenoise-frb/template/`. Generated Dart bindings are never
committed to master — they're regenerated by CI on each run and shipped
inside the published tarball.

| Concern | Location |
|---|---|
| Wrapper crate (Rust) | `crates/whitenoise-frb/` |
| Package template | `crates/whitenoise-frb/template/` |
| FRB config | `flutter_rust_bridge.yaml` |
| Hosted pub registry | `gh-pages` branch → `https://marmot-protocol.github.io/whitenoise-rs/` |
| Local staging | `.frb-staging/` (gitignored; built by `just frb-stage`) |
| Local regen | `just frb-stage && just frb-generate` |
| Wrapper lint | `just frb-check` |
| Local dry-run publish | `just frb-publish-local` |
| CI workflow | `.github/workflows/frb-codegen.yml` |

Versioning: `0.0.0-dev+<short-sha>` per master commit. The Flutter app pins
to a specific version in its pubspec.

Full architecture, migration sequence, and bootstrap instructions:
